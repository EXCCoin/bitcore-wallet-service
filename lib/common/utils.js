...
Utils.formatAmount = function(exels, unit, opts) {
  var UNITS = {
    excc: {
      toexels: 100000000,
      maxDecimals: 6,
  ...
  var amount = (exels / u.toexels).toFixed(u.maxDecimals);
  return addSeparators(amount, opts.thousandsSeparator || ',', opts.decimalSeparator || '.', u.minDecimals);
};

Utils.formatAmountInExcc = function(amount) {
  return Utils.formatAmount(amount, 'excc', {
    minDecimals: 8,
    maxDecimals: 8,
  }) + 'excc';
};
...
Utils.formatUtxos = function(utxos) {
  ...
  return _.map([].concat(utxos), function(i) {
    var amount = Utils.formatAmountInExcc(i.exels);
