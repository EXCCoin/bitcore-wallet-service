...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  opts.network = opts.network || 'exccdlivenet';
  if (!_.contains(['exccdlivenet', 'exccdtestnet'], opts.network))
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['excc', 'dbit'], value.toLowerCase());
    },
  }];
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  bc.getUtxos(addresses, function(err, utxos) {
    ...
    var utxos = _.map(utxos, function(utxo) {
      var u = _.pick(utxo, ['txid', 'vout', 'address', 'scriptPubKey', 'amount', 'exels', 'confirmations']);
      u.confirmations = u.confirmations || 0;
      u.locked = false;
      u.exels = _.isNumber(u.exels) ? +u.exels : Utils.strip(u.amount * 1e8);
      delete u.amount;
      return u;
    });
...
WalletService.prototype._totalizeUtxos = function(utxos) {
  var balance = {
    totalAmount: _.sum(utxos, 'exels'),
    lockedAmount: _.sum(_.filter(utxos, 'locked'), 'exels'),
    totalConfirmedAmount: _.sum(_.filter(utxos, 'confirmations'), 'exels'),
    lockedConfirmedAmount: _.sum(_.filter(_.filter(utxos, 'locked'), 'confirmations'), 'exels'),
  };
...
WalletService.prototype._getBalanceFromAddresses = function(addresses, cb) {
  ...
  self._getUtxosForCurrentWallet(addresses, function(err, utxos) {
    ...
    _.each(utxos, function(utxo) {
      byAddress[utxo.address].amount += utxo.exels;
    });
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self._getUtxosForCurrentWallet(null, function(err, utxos) {
      ...
      var partitionedByAmount = _.partition(inputs, function(input) {
        return input.exels > feePerInput;
      });

      info.utxosBelowFee = partitionedByAmount[1].length;
      info.amountBelowFee = _.sum(partitionedByAmount[1], 'exels');
      ...
      _.each(inputs, function(input, i) {
        ...
        if (sizeInKb > Defaults.MAX_TX_SIZE_IN_KB) {
          ...
          info.amountAboveMaxSize = _.sum(_.slice(inputs, i), 'exels');
          return false;
        }
      ...
      info.amount = _.sum(txp.inputs, 'exels') - info.fee;
 ...
 * @returns {Object} feeLevels - A list of fee levels & associated amount per kB in exel.
 */
WalletService.prototype.getFeeLevels = function(opts, cb) {
  ...
  var network = opts.network || 'exccdlivenet';
  if (network != 'exccdlivenet' && network != 'exccdtestnet')
    return cb(new ClientError('Invalid network'));
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  function sanitizeUtxos(utxos) {
    ...
    return _.filter(utxos, function(utxo) {
      ...
      if (utxo.exels <= feePerInput) return false;
  ...
  function select(utxos, cb) {
    var totalValueInUtxos = _.sum(utxos, 'exels');
    var netValueInUtxos = totalValueInUtxos - baseTxpFee - (utxos.length * feePerInput);

    if (totalValueInUtxos < txpAmount) {
      log.debug('Total value in all utxos (' + Utils.formatAmountInExcc(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInExcc(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      log.debug('Value after fees in all utxos (' + Utils.formatAmountInExcc(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInExcc(txpAmount) + ')');
    ...
    log.debug('Big input threshold ' + Utils.formatAmountInExcc(bigInputThreshold));

    var partitions = _.partition(utxos, function(utxo) {
      return utxo.exels > bigInputThreshold;
    });

    var bigInputs = _.sortBy(partitions[0], 'exels');
    ...
    _.each(smallInputs, function(input, i) {
      ...
      var netInputAmount = input.exels - feePerInput;

      log.debug('The input contributes ' + Utils.formatAmountInExcc(netInputAmount));
      ...
      log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInExcc(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInExcc((selected.length - 1) * feePerInput));
      ...
      log.debug('Cumuled total so far: ' + Utils.formatAmountInExcc(total) + ', Net total so far: ' + Utils.formatAmountInExcc(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        log.debug('Tx change: ', Utils.formatAmountInExcc(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Bitcore.Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          log.debug('Change below dust threshold (' + Utils.formatAmountInExcc(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      log.debug('Could not reach Txp total (' + Utils.formatAmountInExcc(txpAmount) + '), still missing: ' + Utils.formatAmountInExcc(txpAmount - netTotal));
  ...
  log.debug('Selecting inputs for a ' + Utils.formatAmountInExcc(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet(null, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        log.debug('Fee for this selection: ' + Utils.formatAmountInExcc(fee));
      ...
      if (!err) {
        var change = _.sum(txp.inputs, 'exels') - _.sum(txp.outputs, 'amount') - txp.fee;
        log.debug('Successfully built transaction. Total fees: ' + Utils.formatAmountInExcc(txp.fee) + ', total change: ' + Utils.formatAmountInExcc(change));
      } else {
 ...
 * @param {number} opts.amount || opts.outputs[].amount - Amount to transfer in exel.
 ...
 * @param {number} opts.outputs[].amount - Amount to transfer in exel.
 * @param {string} opts.outputs[].message - A message to attach to this output.
 * @param {string} opts.message - A message to attach to this transaction.
