...
helpers.toexel = function(excc) {
  if (_.isArray(excc)) {
    return _.map(excc, helpers.toexel);
  } else {
    return Utils.strip(excc * 1e8);
  }
};
...
helpers._parseAmount = function(str) {
  ...
  var re = /^((?:\d+c)|u)?\s*([\d\.]+)\s*(excc|bit|sat)?$/;
...
helpers.createProposalOpts = function(type, outputs, signingKey, moreOpts, inputs) {
  _.each(outputs, function(output) {
    output.amount = helpers.toexel(output.amount);
  });
